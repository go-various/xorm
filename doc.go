...
1. query with a SQL string, the returned results is []map[string][]byte
	...
	results, err := engine.Prompt("select * from user")
...
2. Prompt one record or one variable from database
...
3. Prompt multiple records from database
...
4. Prompt multiple records and record by record handle, there two methods, one is Iterate,
another is Rows
...
6. With Prompt
	...
	var query = builder.Select("name").From("group")
	results, err := engine.Prompt(query)
...
More usage, please visit http://github.com/go-various/docs
*/
package xorm
